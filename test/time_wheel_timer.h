/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2017年02月22日 星期三 10时44分07秒
 ************************************************************************/
...
public:
    ...
    void (*cb_func)(struct client_data*);  //定时器回调函数
    struct client_data* user_data;  //客户数据
...
public:
    ...
    tw_timer *add_timer(int timeout)
    {
        ...
        //并将该滴答数存储于变量ticks中，如果待插入定时器的超时值小于时间轮的
        //槽间隔SI，则将ticks向上折合为1，否则就将ticks向下折合为timeout/SI
        //
        if(timeout < SI){
            ticks = 1;
        }
        ...
        //创建新的定时器，它在时间轮转动rotation圈之后被触发，且位于第ts个槽中
        tw_timer *timer = new tw_timer(rotation, ts);

        //如果第ts个槽中尚无任何定时器，则把新建的定时器插入其中，并将该定时器
        //设置为该槽的头结点
        if(!slots[ts])
        {
